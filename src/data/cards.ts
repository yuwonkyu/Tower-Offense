...
/**
 * 카드 풀 고갈(모든 슬롯 만렙) 시 잉여 레벨업 선택권 → 골드/다이아 자동 지급.
 * 선택 UI 없이 안내 1회 후 자동 습득 (battleStore에서 처리).
 */
export const RESOURCE_REWARD = { gold: 100, diamonds: 1 } as const;

/** MAX 도달 시 추가 카드 선택권 1회, 재화 자동 지급 1회당 다이아 확률 5% (그 외 골드) */
export const CARD_BONUS = { maxLevelExtraPick: 1, bonusPickChance: 0.05 } as const;

/** 재화 자동 지급 1회 — 5% 다이아, 95% 골드 */
export function rollResourceReward(): { gold: number; diamonds: number } {
  return Math.random() < CARD_BONUS.bonusPickChance
    ? { gold: 0, diamonds: RESOURCE_REWARD.diamonds }
    : { gold: RESOURCE_REWARD.gold, diamonds: 0 };
}

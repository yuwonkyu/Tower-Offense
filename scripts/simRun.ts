/**
 * 헤드리스 전투 시뮬 공용 모듈 — 단판 실행 + "괜찮은 플레이어" 픽 모델.
 * simBattle.ts(단판 상세 로그)와 winrate.ts(배치 승률)가 공유한다.
 */
 ...
 */
export const PICK_WEIGHT: Record<string, number> = {
  // 핵심 성장
  g_exp: 10,
  g_spawnspeed: 9,
  g_atk: 8,
...
export function autoPick(engine: BattleEngine, pickLog?: Map<string, number>) {
...
/** 메타 성장 프로필을 입힌 영웅/유닛 보너스/해금 목록 생성 (스테이지 20+ 진척 근사). */
export function buildMetaProfile(metaLevel: number): {
  heroDef: HeroDef;
  ...
  return { heroDef, unitMetaBonuses, unlockedUnits };
}
...
/** 단판 시뮬 실행. verbose=true면 헤더 + 1분 간격 전선 로그 출력. */
export function runSim(stage: number, metaLevel = 0, verbose = false): SimResult {
  const config = getStageConfig(stage);
  const { heroDef, unitMetaBonuses, unlockedUnits } = buildMetaProfile(metaLevel);
  ...
  while (elapsed < config.timeLimit && engine.result === 'ongoing') {
    autoPick(engine, pickLog);
    engine.useHeroSkill(); // 준비되면 즉시 사용 (쿨다운/타깃 없음 시 무시)
    engine.tick(DT);
    elapsed += DT;
    if (verbose && Math.abs(elapsed % 60) < DT) {
  ...
  return {
    result: engine.result as SimResult['result'],
    win: engine.result === 'victory',
    elapsed,
    level: engine.level,
    kills: engine.kills,
    totalExp: engine.totalExp,
    towerHp: engine.towerHp,
    towerPct: (engine.towerHp / config.tower.hp) * 100,
    slotsUsed: engine.cards.slotsUsed,
    picks,
  };
}
